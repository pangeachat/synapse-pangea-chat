...
Both tiers ship dark: nothing runs until an operator enables a tier in the module's `moderation` config, and Tier 2 refuses to start half-configured — base URL and service token are required at parse time, and the base URL must be a fetchable `http`/`https` URL: ASCII, with a host, no whitespace, no embedded credentials, and no `?` or `#` anywhere in it. The last one is not pedantry — `urlparse` reports an empty query and an empty fragment for a trailing `?` or `#`, both falsy, and the endpoint path is appended to the value, so `https://choreo.invalid?` sends every check to `?/choreo/moderate` and `#` swallows the path entirely. A non-empty string that is not a URL (`ftp://choreo.invalid`, or a bare hostname) used to start cleanly and then fail on every message inside the fail-open handler, which is indistinguishable from moderation being switched off. A plaintext `http` base URL parses, for local stacks, and logs a WARNING: the service token and every moderated message cross the network in the clear.
