...
#: How long one send holds the notice before another may try it. Well past the
#: mailer's own bound (SEND_TIMEOUT_SECONDS), so no send is still being waited
#: on when the next may start. A send given up on can still deliver if the mail
#: server recovers, so a retry after a stall can repeat the email: accepted,
#: because the email carries nothing single use, and losing it would leave the
#: teacher without the link to share.
