...
| Module outbound HTTP calls (found in production, SYNAPSE-BT, #214) | The second half of the row above: a handler launched correctly still leaks if it awaits a raw Twisted Deferred. `direct_push` awaited `Agent.request()`/`readBody()` directly, so every `send_push` leaked for as long as Sygnal took to answer and killed the timers firing in that window (39 in production on 2026-09-20) | All 5 raw `Agent` call sites (`direct_push`, `public_courses/course_plan_l2_lookup`, 3 in `export_user_data`) wrap each awaited Deferred in `make_deferred_yieldable`; `tests/test_logcontext_e2e.py` drives `send_push` against a stub Sygnal that holds each answer, since an instant answer leaves no window to catch |
