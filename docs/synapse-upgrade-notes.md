...
| Module outbound HTTP calls (found in production, SYNAPSE-BT, #214) | The second half of the row above: a handler launched correctly still leaks if, after an earlier Synapse-aware await has resumed it from the reactor, it awaits a raw Twisted Deferred. `direct_push` awaited `Agent.request()`/`readBody()` directly after its DB reads, so every `send_push` handed the request's context to the reactor for as long as Sygnal took to answer. Every `looping_call` firing in that window stopped for good (39 in production on 2026-09-20), and every one-shot `call_later` firing in it never ran, silently, since only the former logs | All 5 raw `Agent` call sites (`direct_push`, `public_courses/course_plan_l2_lookup`, 3 in `export_user_data`) wrap each awaited Deferred in `make_deferred_yieldable`. `tests/test_logcontext_e2e.py` drives `send_push` against a stub Sygnal that holds each answer (an instant answer leaves no window to catch), and its markers include the tail every `clock.py` sentinel assert shares, so a lost one-shot timer fails the gate too. Timers already lost come back only on a Synapse restart; an ansible run without `start` / `--tags=restart` does not restart it |
