...
The importer preserves the brand markup and adapts only the template-engine slots, message body, removal of campaign receiving-reason copy, unsubscribe link, and configurable postal address. It fails if expected source slots change. `brand_base.html` and its shared `brand_footer.html` are generated; edit the admin source and reimport for brand changes. Nudge-specific content lives in `nudge_email.html`. The preview's default postal address is imported from the source footer; deployed delivery continues to use its required configuration value.
